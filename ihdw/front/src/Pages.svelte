<script>
	...
	async function getCategories () { if (api) {
		message = ''
	...
	async function savePage () {
		api.addOrUpdatePage(cat_selected_used, page_config.url, page_config.template).then(x => {message = x; new_page=''; getCategories()}).catch(x => message =x)
	}

	async function deletePage() {
		message = await api.deletePage(cat_selected_used).then(x => {message = ''; getCategories();}).catch(x => message = x)
	}
...
<aside>
	<h1>Pages</h1>
	Category : 
	<select bind:value={cat_selected_used} on:change={onSelectCategory}>
			<option value=''>
			</option>
